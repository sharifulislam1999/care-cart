        ...
        if(photo.length){
           ...
           if(uploadResponse.data.success){
            if(updateBgImg.length){
                const updateBgImg = {image:data.upmedicineBgImage[0]}
                const upUploadResponse = await axiosPublic.post(image_upload_api,updateBgImg,{
                    headers:{
                        'content-type':'multipart/form-data'
                    }
                   });
                   if(upUploadResponse.data.success){
                    const medicine = {
                        medicineName: data.upmedicineName,
                        genericName: data.upmedicineGeneric,
                        medicineImage: uploadResponse.data.data.display_url,
                        medicineBgImage:upUploadResponse.data.data.display_url,
